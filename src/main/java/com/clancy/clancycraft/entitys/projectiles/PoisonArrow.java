...
public class PoisonArrow extends CCThrowable {
    private int explosionPower = 1;
    private int zoneTimer = 80;
    private boolean hasHit;

    public PoisonArrow(EntityType<? extends PoisonArrow> type, Level world) {
        super(type, world);
    }

    public PoisonArrow(EntityType<? extends PoisonArrow> type, Level world, LivingEntity thrower) {
    ...
    public void makeTrail() {
        BlockState stateId = Blocks.GREEN_CONCRETE_POWDER.defaultBlockState();
        for (int i = 0; i < 5; i++) {
        ...
        for (LivingEntity entity : nearby) {
            if (entity != this.getOwner()) {
                if (entity instanceof LivingEntity) {
                    ...
                    level.setBlockAndUpdate(pos, Blocks.GREEN_CONCRETE_POWDER.defaultBlockState());
                    level.setBlockAndUpdate(pos.above(), Blocks.GREEN_CONCRETE_POWDER.defaultBlockState());
